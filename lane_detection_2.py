...
def process_an_image(img):
    ...
    dst = clahe.apply(gray)#直方图局部自适应均衡化
    cv2.imshow("dst", dst)
    plt.hist(dst.ravel(), 256, (0, 256))
    plt.show()

    blur_gray = cv2.GaussianBlur(dst, (blur_ksize, blur_ksize), 1)
    #blur_gray = cv2.blur(gray,(5,5))#均值模糊
    #blur_gray = cv2.medianBlur(gray,5)#中值滤波
    cv2.imshow("blur_gray", blur_gray)#高斯滤波
    ...
    points = np.array([[(0, 500), (480, 325), (520, 325), (960, 500)]])
    # [[[0 540], [460 325], [520 325], [959 540]]]
...
def least_squares_fit(point_list, ymin, ymax):
    ...
    fit = np.polyfit(y, x, 1)#用一次多项式x=a*y+b拟合这些点，fit是(a,b)，1代表线性
